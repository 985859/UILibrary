...
public class PointLengthFilter implements InputFilter {
    ...
    //输入的最大金额
    private   double MAX_VALUE = 99999999.99;
    //小数点后的位数
    private   int POINTER_LENGTH = 2;
    public void setMAX(double MAX_VALUE) {
        this.MAX_VALUE = MAX_VALUE;
    }
    public void setPointerLength(int POINTER_LENGTH) {
    ...
    public CharSequence filter(CharSequence source, int start, int end, Spanned dest, int dstart, int dend) {
        ...
        if(destText.contains(POINTER)) {
            ...
            if (!matcher.matches()) {
                ...
                if ((POINTER.equals(source.toString())) && StringUtil.isEmpty(destText)) {  //首位不能输入小数点
                    return "";
