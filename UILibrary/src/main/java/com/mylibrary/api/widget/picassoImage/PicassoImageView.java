...
public class PicassoImageView extends AppCompatImageView {
    ...
    private ColorStateList mBorderColor =
            ColorStateList.valueOf(PicassoDrawable.DEFAULT_BORDER_COLOR);
    ...
    public PicassoImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PicassoImageView, defStyle, 0);

        int index = a.getInt(R.styleable.PicassoImageView_android_scaleType, -1);
        if (index >= 0) {
        ...
        float cornerRadiusOverride =
                a.getDimensionPixelSize(R.styleable.PicassoImageView_attr_cornersRaius, -1);

        mCornerRadii[Corner.TOP_LEFT] =
                a.getDimensionPixelSize(R.styleable.PicassoImageView_attr_topLeftRadius, -1);
        mCornerRadii[Corner.TOP_RIGHT] =
                a.getDimensionPixelSize(R.styleable.PicassoImageView_attr_topRightRadius, -1);
        mCornerRadii[Corner.BOTTOM_RIGHT] =
                a.getDimensionPixelSize(R.styleable.PicassoImageView_attr_bottomRightRadius, -1);
        mCornerRadii[Corner.BOTTOM_LEFT] =
                a.getDimensionPixelSize(R.styleable.PicassoImageView_attr_bottomLeftRadius, -1);
        ...
        mBorderWidth = a.getDimensionPixelSize(R.styleable.PicassoImageView_attr_strokeWidth, -1);
        if (mBorderWidth < 0) {
            mBorderWidth = DEFAULT_BORDER_WIDTH;
        }

        mBorderColor = a.getColorStateList(R.styleable.PicassoImageView_attr_strokeColor);
        if (mBorderColor == null) {
            mBorderColor = ColorStateList.valueOf(PicassoDrawable.DEFAULT_BORDER_COLOR);
        }

        mMutateBackground = a.getBoolean(R.styleable.PicassoImageView_attr_mutate_background, false);
        mIsOval = a.getBoolean(R.styleable.PicassoImageView_attr_oval, false);

        final int tileMode = a.getInt(R.styleable.PicassoImageView_attr_tile_mode, TILE_MODE_UNDEFINED);
        ...
        final int tileModeX =
                a.getInt(R.styleable.PicassoImageView_attr_tile_mode_x, TILE_MODE_UNDEFINED);
        if (tileModeX != TILE_MODE_UNDEFINED) {
            setTileModeX(parseTileMode(tileModeX));
        }

        final int tileModeY =
                a.getInt(R.styleable.PicassoImageView_attr_tile_mode_y, TILE_MODE_UNDEFINED);
    ...
    public void setImageUrl(String url, int placeholder) {
        setImageUrl(url, placeholder, -1);
    }

    public void setImageUrl(String url, int placeholder, int errorID) {
        if (StringUtil.isEmpty(url)) {
            //url的长度 必须大于0  否者会报异常   防止url为空 或为空字符 不加载位图  错误图
            url = "http";
        }

        RequestBuilder builder = Glide.with(context).load(url).fitCenter().diskCacheStrategy(DiskCacheStrategy.RESOURCE);
        if (placeholder != -1) {
    ...
    public void setImageDrawable(Drawable drawable) {
        mResource = 0;
        mDrawable = PicassoDrawable.fromDrawable(drawable);
        updateDrawableAttrs();
    ...
    public void setImageBitmap(Bitmap bm) {
        mResource = 0;
        mDrawable = PicassoDrawable.fromBitmap(bm);
        updateDrawableAttrs();
        super.setImageDrawable(mDrawable);
    }
    ...
    private Drawable resolveResource() {
        ...
        return PicassoDrawable.fromDrawable(d);
    }
    ...
    private void updateBackgroundDrawableAttrs(boolean convert) {
        if (mMutateBackground) {
            if (convert) {
                mBackgroundDrawable = PicassoDrawable.fromDrawable(mBackgroundDrawable);
            }
    ...
    private void updateAttrs(Drawable drawable, ScaleType scaleType) {
        ...
        if (drawable instanceof PicassoDrawable) {
            ...
            if (mCornerRadii != null) {
                ((PicassoDrawable) drawable).setCornerRadius(
                        mCornerRadii[Corner.TOP_LEFT],
                        mCornerRadii[Corner.TOP_RIGHT],
                        mCornerRadii[Corner.BOTTOM_RIGHT],
                        mCornerRadii[Corner.BOTTOM_LEFT]);
    ...
    public void setBorderColor(ColorStateList colors) {
        ...
        mBorderColor =
                (colors != null) ? colors : ColorStateList.valueOf(PicassoDrawable.DEFAULT_BORDER_COLOR);
        updateDrawableAttrs();
        updateBackgroundDrawableAttrs(false);
        if (mBorderWidth > 0) {
            invalidate();
     ...
     * width.
     *
     * @return if this {@link PicassoImageView} is set to oval.
     */
     ...
     *
     * @param oval if this {@link PicassoImageView} should be oval.
     */
    ...
    /**
     * Set whether the {@link PicassoImageView} should round the background drawable according to
